...
def _record(metric_id: str) -> dict:
    ...
    value: float | bool | int = 1 if value_type == "count" else (True if value_type == "boolean" else 1.0)
    return {
        "metric_id": metric_id,
...
def test_missing_required_aggregate_metric_is_rejected() -> None:
    ...
    with pytest.raises(MetricContractError, match="missing consensus counts"):
        _build(document)
...
def test_metric_evaluator_must_match_dictionary_source() -> None:
    document = _metrics_document()
    document["records"][0]["evaluator"] = "truvari"
...
def test_cli_scores_only_bound_standard_metrics_and_preserves_raw_f1(
    ...
    assert output["consensus_counts"] == {
        "all_three_correct": 1,
        "exactly_two_correct": 1,
        "exactly_one_correct": 1,
        "none_correct": 1,
