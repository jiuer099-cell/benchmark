#!/usr/bin/env python3
"""Read-only verification of the bound Obsidian canonical design note."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        write_check(args.output, result)
        print(
            f"check_obsidian_sync: {result['status']} "
            f"mismatches={len(result['mismatches'])}"
        )
        return 0 if args.allow_stale or result["status"] == "current" else 1
    except (OSError, ObsidianSyncError, ValueError) as exc:
        print(f"check_obsidian_sync: {exc}", file=sys.stderr)
        return 2
