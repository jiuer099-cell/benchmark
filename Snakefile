...
for registration in config["external_plugins"]:
    ...
    if OFFICIAL_MODE == "end_to_end_from_reads":
        settings["inputs"] = {
            "canonical_fastq": config["development"]["canonical_fastq"],
            "reference": config["reference"]["fasta"],
            "pangenome_manifest": (
                f"results/pangenome/{PANGENOME_ID}/manifest.yaml"
            ),
...
if SYNTHETIC_MODE:
    tool_id = EXTERNAL_SETTINGS[0]["tool_id"]
    FINAL_TARGETS = [
        ...
        f"results/summary/{tool_id}/score.json",
        f"results/summary/{tool_id}/score-package.json",
        f"results/summary/{tool_id}/metrics.json",
        f"results/{SAMPLE_ID}/{OFFICIAL_MODE}/{tool_id}/canonical/linked.vcf",
        f"results/{SAMPLE_ID}/{OFFICIAL_MODE}/{tool_id}/canonical/allele_links.tsv",
        f"results/provenance/{tool_id}/pre-score-audit.json",
        f"results/provenance/{tool_id}/pre-score-lineage.json",
        f"results/provenance/{tool_id}/pre-score-lineage.tsv",
        f"results/provenance/{tool_id}/rule-lineage.json",
        f"results/provenance/{tool_id}/rule-lineage.tsv",
        f"results/provenance/{tool_id}/provenance-audit.json",
    ]
